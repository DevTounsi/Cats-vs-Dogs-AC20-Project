...
Ce dépôt contient le code source et les expériences liés à mon projet de l'Unité de Valeur **AC20** (Connaissances Scientifiques) à l'**Université de Technologie de Belfort-Montbéliard (UTBM)**.
...
```text
├── data/                   # Dossier contenant le dataset (Kaggle Dogs vs Cats) - non versionné
├── notebooks/              # Jupyter Notebooks pour l'exploration (EDA) et les expérimentations
├── src/                    # Scripts Python (définition des modèles, boucles d'entraînement)
...
Sous la supervision de **M. Abdeljalil ABBAS-TURKI**.
